...
func TestRevokeTokenWithoutStorage(t *testing.T) {
	t.Parallel()

	// Token generated by the auth helper, so it carries a JTI claim.
	jwtAuth := auth.NewJWT(getTestSigningKey())
	token, err := jwtAuth.Create("user-rev", time.Hour)
	require.NoError(t, err)

	svc := NewHandlers(Options{SigningKey: getTestSigningKey()})

	req := httptest.NewRequest(http.MethodPost, "/api/user/token/revoke", nil)
	req.Header.Set("Authorization", "Bearer "+token)
	w := httptest.NewRecorder()

	svc.RevokeToken(w, req)

	// Without storage wired, revoke surfaces a 503 instead of pretending success.
	require.Equal(t, http.StatusServiceUnavailable, w.Code)
}

func TestRevokeTokenMissingAuthorization(t *testing.T) {
	t.Parallel()
